...
fn read_file(
    ...
    let lines = filesystem::read_lines(path)?;
    read_lines(lines, path, variables, visited_lines, writer, stdin)
}
...
fn read_tldr_markdown(
    ...
    read_lines(prefix.chain(lines), "markdown", variables, visited_lines, writer, stdin)
}
...
pub fn read_all(
    ...
    // TODO: remove
    read_tldr_markdown(&mut variables, &mut visited_lines, writer, stdin);
    return Ok(variables);
